...
if eval pit_check; then
	echo "This script should be used post-livecd reboot."
	exit 1
fi
...
if ! eval check_cray_init_status >/dev/null 2>&1; then
    ...
    CRAY_FORMAT=toml initrd_path=$(cray bss bootparameters list --hosts "${xname}" | awk -F '"' '/initrd = / {print $2}')
		...
		scp $SCRIPT_DIR/validate-bootraid-artifacts-lib.sh $ncn:$SCRIPT_DIR
		# run the script, fix kernel and initrd
		ssh $ncn "chmod 755 $SCRIPT_DIR/validate-bootraid-artifacts-lib.sh"
		ssh $ncn "$SCRIPT_DIR/validate-bootraid-artifacts-lib.sh ${kernel_path}"
		ssh $ncn "$SCRIPT_DIR/validate-bootraid-artifacts-lib.sh ${initrd_path}"
